...
const reportEthBalances = async () => {
  const l1Balance = await l1RpcProvider.getBalance(ourAddr);
  const l2Balance = await l2RpcProvider.getBalance(ourAddr);
  console.log(`l1 eth: ${l1Balance} l2 eth: ${l2Balance}`);
}
...
const depositEth = async () => {
  ...
  const response = await crossChainMessenger.depositETH(ethers.utils.parseEther("0.001"));
  console.log(`Deposit transaction hash (on L1): ${response.hash}`);
  console.log(`\tMore info: https://goerli.etherscan.io/tx/${response.hash}`)
  await response.wait();
  console.log("Waiting for status to change to RELAYED")
